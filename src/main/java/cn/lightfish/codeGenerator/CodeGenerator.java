package cn.lightfish.codeGenerator;
...
public class CodeGenerator {
    ...
    public CodeGenerator(Context yaccFile, LRParsingTable parsingTable) {
        ...
        for (Symbol symbol : yaccFile.getSymbols()) {
            ...
            for (Symbol actionItem : entry.actions.keySet()) {
                ...
                if (actionStr.contains("S")) {
                    ...
                    if (actionStr.contains("accept")) {
                        ...
                        int symbolsNum = production.getRights().size();
                        ...
                        String thisSymbolName = production.getLeft().toString();
                        FuncCallBlock callBlock = new FuncCallBlock("translate_" + production.getId());
                        callBlock.putArg(thisSymbolName);
                        List<String> reversed = new ArrayList<>();
                        keptSymbols.add(thisSymbolName);
                        caseInnerBlock.putBlock(production.getLeft().getClassName() + " " + thisSymbolName + " = new " + production.getLeft().getClassName() + "();");
                        for (int j = production.getRights().size() - 1; j >= 0; j--) {
                            thisSymbolName = production.getRights().get(j).toString();
                            if (keptSymbols.contains(thisSymbolName)) {
                                thisSymbolName = thisSymbolName + "_" + keptSymbols.lastIndexOf(thisSymbolName);
                            }
                            caseInnerBlock.putBlock(production.getRights().get(j).getClassName() + " " + thisSymbolName + " = (" + production.getRights().get(j).getClassName() + ")symbolStack.pop();");
                            keptSymbols.add(thisSymbolName);
...
//                            callBlock.putArg(thisSymbolName);
                        ...
                        pushAndGotoCall.putArg(production.getLeft().toString());
...
//        }
        yaccFile.getProductions().remove(yaccFile.getProductions().size() - 1);

        for (Production production : yaccFile.getProductions()) {
            FuncDefBlock funcDefBlock = new FuncDefBlock("translate_" + production.getId());
            String left;
            if (production.getRights().contains(production.getLeft())) {
                left = production.getLeft().toString() + "_l";
            } else {
                left = production.getLeft().toString();
            }
            funcDefBlock.putParamater(left, production.getLeft().getClassName());
            FuncCallBlock defaultOutput = new FuncCallBlock("output");
            defaultOutput.putArg(left);
            List<String> keptSymbols = new ArrayList<>();
            for (Symbol right : production.getRights()) {
        ...
        for (Production production : yaccFile.getProductions()) {
            ...
            FuncCallBlock callBlock = new FuncCallBlock("translate_" + production.getId());
            List<String> keptSymbols = new ArrayList<>();
            callBlock.putArg(production.getLeft().toString());
            List<Symbol> rights = new ArrayList<>();
            for (int i = 0; i < production.getRights().size(); i++) {
                String thisSymbolName = production.getRights().get(i).toString();
                if (keptSymbols.contains(thisSymbolName)) {
                    thisSymbolName = production.getRights().get(i).toString() + "_" + keptSymbols.lastIndexOf(thisSymbolName);
                }
                caseBlock.putBlock(production.getRights().get(i).getName().toUpperCase() + " " + thisSymbolName + " = reduce.get(" + i + ");");
                rights.add(production.getRights().get(i));
                keptSymbols.add(production.getRights().get(i).toString());
            }
...
//            template = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream("/template.txt")));
            template = new BufferedReader(new InputStreamReader(new FileInputStream(this.getClass().getResource("/template.txt").getFile())));
